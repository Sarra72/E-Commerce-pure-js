...
function productItem(i){
    return  `
            <div class="product-item col-12 col-sm-6 col-md-4 x mb-4 p-4 ">
                <div class="card border border-info pt-3">
                    <img class="product-item-img card-img-top m-auto" src="${i.imageURL}" alt="Card image" style="width:80%; height:${heightImage};">
                    <div class="product-itm-desc card-body pb-0 pl-4">
                        <p class="card-title ">Product: ${i.title}.</p>
                        <p class="card-text">Category :${i.category}.</p>
                        <p class="color">Color: ${i.color}.</p>
                        <p class="card-price">Price: <span> <del>${i.price} EGP</del> ${i.salePrice} EGP</span></p>
                    </div>
                    <div class="product-item-action d-flex justify-content-between pr-4 pl-4">
                    <button id="add-btn-${i.id}" class="AddToCartBtn btn btn-primary mb-2" onClick="addTOCartEvent(${i.id})">Add To Cart</button>
                    <button id="remove-btn-${i.id}" class="RemoveFromCartBtn btn btn-primary mb-2" onClick="removeFromCart(${i.id})">Remove From Cart</button>
                        <i id="fav-${i.id}" class="${isFavorite(i)} fa-heart" onClick="addFav(${i.id})"></i>
                    </div>
                </div>
            </div>
        `
...
let productInNavCart = document.querySelector(".product-in-nav-cart");
...
function updateBadge() {
    ...
    addItemStorage.forEach((item) => {
        totalItems += +(localStorage.getItem(`quantity-${item.id}`)) || 0;
    });
...
function pls(id, salePrice) {
    ...
    // update total num of products in cart badge======================================================================
    updateBadge();
...
function addTOCartEvent(id) {
    if (localStorage.getItem("userName")) {
        ...
        if (itemIndex === -1) {
            ...
            localStorage.setItem(`quantity-${choosenItem.id}`, quantity.toString());
...
//draw product item in nav cart 
function drawBuyProudect(item) {
    //Runs the block of code only if the element with that id is NOT found in the DOM 
    if (!document.getElementById(`buyProudectItem-${item.id}`)) {
        let quantity = +(localStorage.getItem(`quantity-${item.id}`)) || 1;

        productInNavCart.innerHTML += `<div id="buyProudectItem-${item.id}" class="row my-2 pr-2">
...
//toggle between a filled heart or empty heart
function isFavorite(i){
    let isFavorite = checkFavorite(i.id);
    let heartIconClass = isFavorite ? "fas" : "far";
    return heartIconClass;
}
...
//chech if id not in fav list push it and update local storage
...
//remove from fav and update local storage 
...
//this func work on seacrch input field
